...
use App\Exports\SchoolBoardsExport;
use Maatwebsite\Excel\Facades\Excel;

class SchoolBoardRepository implements SchoolBoardInterface
{
    ...
    public function store(array $array)
    {
        ...
        try {
            ...
            $lastPosition = SchoolBoard::max('position');
    ...
    {
        try {
            $data = SchoolBoard::where('slug', $slug)->first();
        ...
        try {
            ...
            if ($data['code'] == 200) {
                ...
                $this->trashRepository->store([
                    'model' => 'SchoolBoard',
                    'table_name' => 'school_boards',
                    'deleted_row_id' => $data['data']->id,
                    'thumbnail' => $data['data']->image_s,
                    'title' => $data['data']->title,
                    'description' => $data['data']->title.' data deleted from school boards table',
                    'status' => 'deleted',
                ]);
    ...
    {
        try {
            $data = SchoolBoard::whereIn('id', $array['ids'])->get();
            if ($array['action'] == 'delete') {
                $data->each(function ($item) {
                    ...
                    $this->trashRepository->store([
                        ...
                        'title' => $item->title,
                        'description' => $item->title.' data deleted from school boards table',
                        'status' => 'deleted',
                    ]);
    ...
    public function import(UploadedFile $file)
    {
        try {
            ...
            // $processedCount = saveToDatabase($data, 'SchoolBoard');
    ...
    public function export(?String $keyword = '', array $filters = [], String $perPage, String $sortBy = 'id', String $sortOrder = 'asc', String $type)
    {
        try {
            ...
            if (count($data['data']) > 0) {
                $fileName = "school_boards_export_" . date('Y-m-d') . '-' . time();

                if ($type == 'excel') {
                    $fileExtension = ".xlsx";
                    return Excel::download(new SchoolBoardsExport($data['data']), $fileName.$fileExtension);
                }
                elseif ($type == 'csv') {
                    $fileExtension = ".csv";
                    return Excel::download(new SchoolBoardsExport($data['data']), $fileName.$fileExtension, \Maatwebsite\Excel\Excel::CSV);
                }
                elseif ($type == 'html') {
                    $fileExtension = ".html";
                    return Excel::download(new SchoolBoardsExport($data['data']), $fileName.$fileExtension, \Maatwebsite\Excel\Excel::HTML);
                }
                elseif ($type == 'pdf') {
                    $fileExtension = ".pdf";
                    return Excel::download(new SchoolBoardsExport($data['data']), $fileName.$fileExtension, \Maatwebsite\Excel\Excel::TCPDF);
                }
                else {
                    return [
                        'code' => 400,
    ...
    public function position(array $ids)
    {
        try {
            foreach ($ids as $index => $id) {
                SchoolBoard::where('id', $id)->update([
                    'position' => $index + 1
                ]);
            }
            SchoolBoard::clearActiveCollectionsCache();
