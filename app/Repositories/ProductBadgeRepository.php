...
use App\Exports\ProductBadgesExport;
use Maatwebsite\Excel\Facades\Excel;

class ProductBadgeRepository implements ProductBadgeInterface
{
    ...
    public function update(Array $array)
    {
        try {
            ...
            if ($data['code'] == 200) {
                $data['data']->category = $array['category'];
                $data['data']->title = $array['title'];
                $data['data']->slug = \Str::slug($array['title']);
                $data['data']->short = $array['short'] ?? null;
                $data['data']->description = $array['description'] ?? null;
                $data['data']->icon = $array['icon'];
                $data['data']->tailwind_classes = $array['tailwind_classes'];
                $data['data']->meta = $array['meta'];
        ...
        try {
            ...
            if ($data['code'] == 200) {
                ...
                $this->trashRepository->store([
                    'model' => 'ProductBadge',
                    'table_name' => 'product_badges',
                    'deleted_row_id' => $data['data']->id,
                    'thumbnail' => $data['data']->icon,
                    'title' => $data['data']->title,
                    'description' => $data['data']->title.' data deleted from product badges table',
                    'status' => 'deleted',
                ]);
    ...
    {
        try {
            $data = ProductBadge::whereIn('id', $array['ids'])->get();
            if ($array['action'] == 'delete') {
                $data->each(function ($item) {
                    ...
                    $this->trashRepository->store([
                        ...
                        'thumbnail' => $item->icon,
                        'title' => $item->title,
                        'description' => $item->title.' data deleted from product badges table',
                        'status' => 'deleted',
                    ]);
    ...
    public function import(UploadedFile $file)
    {
        try {
            ...
            // $processedCount = saveToDatabase($data, 'ProductBadge');
    ...
    public function export(?String $keyword = '', Array $filters = [], String $perPage, String $sortBy = 'id', String $sortOrder = 'asc', String $type)
    {
        try {
            ...
            if (count($data['data']) > 0) {
                $fileName = "product_badges_export_" . date('Y-m-d') . '-' . time();

                if ($type == 'excel') {
                    $fileExtension = ".xlsx";
                    return Excel::download(new ProductBadgesExport($data['data']), $fileName.$fileExtension);
                }
                elseif ($type == 'csv') {
                    $fileExtension = ".csv";
                    return Excel::download(new ProductBadgesExport($data['data']), $fileName.$fileExtension, \Maatwebsite\Excel\Excel::CSV);
                }
                elseif ($type == 'html') {
                    $fileExtension = ".html";
                    return Excel::download(new ProductBadgesExport($data['data']), $fileName.$fileExtension, \Maatwebsite\Excel\Excel::HTML);
                }
                elseif ($type == 'pdf') {
                    $fileExtension = ".pdf";
                    return Excel::download(new ProductBadgesExport($data['data']), $fileName.$fileExtension, \Maatwebsite\Excel\Excel::TCPDF);
                }
                else {
                    return [
                        'code' => 400,
    ...
    public function position(Array $ids)
    {
        try {
            foreach ($ids as $index => $id) {
                ProductBadge::where('id', $id)->update([
                    'position' => $index + 1
                ]);
            }
