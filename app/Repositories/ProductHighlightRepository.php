...
use App\Exports\ProductHighlightListAttributesExport;
use Maatwebsite\Excel\Facades\Excel;

class ProductHighlightRepository implements ProductHighlightInterface
{
    ...
    public function list(?String $keyword = '', array $filters = [], String $perPage, String $sortBy = 'id', String $sortOrder = 'asc') : array
    {
        try {
            DB::enableQueryLog();
            $query = ProductHighlightList::query();
    ...
    public function store(array $array)
    {
        ...
        try {
            ...
            $lastPosition = ProductHighlightList::where('product_id', $productId)
            ->max('position');
            ...
            if (!empty($array['variations']) && count($array['variations']) > 0) {
                ...
                foreach ($array['variations'] as $key => $variation) {
                    $this->ProductHighlightListCombinationRepository->store([
    ...
    protected function checkExistingVariation($productId, array $variations)
    {
        ...
        $variationIds = ProductHighlightList::where('product_id', $productId)
            ->pluck('id');
        ...
        foreach ($variations as $variation) {
            $variationIds = ProductHighlightListCombination::whereIn('variation_id', $variationIds)
        ...
        return ProductHighlightList::with('combinations')
    ...
    {
        try {
            $data = ProductHighlightList::where('id', $id)
    ...
    {
        try {
            $data = ProductHighlightList::whereIn('id', $array['ids'])->get();
    ...
    public function import(UploadedFile $file)
    {
        try {
            ...
            // $processedCount = saveToDatabase($data, 'ProductHighlightList');
            ...
            foreach ($data as $item) {
                ...
                $createdData = ProductHighlightList::create([
                ...
                if (!empty($item['values']) && count($createdData->values) == 0) {
                    ...
                    foreach ($explodedValues as $key => $singleValue) {
                        $dd = $this->ProductHighlightListAttributeValueRepository->store([
    ...
    public function export(?String $keyword = '', array $filters = [], String $perPage, String $sortBy = 'id', String $sortOrder = 'asc', String $type)
    {
        try {
            ...
            if (count($data['data']) > 0) {
                ...
                if ($type == 'excel') {
                    $fileExtension = ".xlsx";
                    return Excel::download(new ProductHighlightListAttributesExport($data['data']), $fileName.$fileExtension);
                }
                elseif ($type == 'csv') {
                    $fileExtension = ".csv";
                    return Excel::download(new ProductHighlightListAttributesExport($data['data']), $fileName.$fileExtension, \Maatwebsite\Excel\Excel::CSV);
                }
                elseif ($type == 'html') {
                    $fileExtension = ".html";
                    return Excel::download(new ProductHighlightListAttributesExport($data['data']), $fileName.$fileExtension, \Maatwebsite\Excel\Excel::HTML);
                }
                elseif ($type == 'pdf') {
                    $fileExtension = ".pdf";
                    return Excel::download(new ProductHighlightListAttributesExport($data['data']), $fileName.$fileExtension, \Maatwebsite\Excel\Excel::TCPDF);
                }
                else {
                    return [
                        'code' => 400,
    ...
    public function position(array $ids)
    {
        try {
            ...
            foreach ($ids as $index => $id) {
                $resp = ProductHighlightList::where('id', $id)->update([
                    'position' => $index + 1
                ]);
            }
