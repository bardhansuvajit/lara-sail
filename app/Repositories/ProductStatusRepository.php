...
use App\Exports\ProductStatusesExport;
use Maatwebsite\Excel\Facades\Excel;

class ProductStatusRepository implements ProductStatusInterface
{
    ...
    public function store(array $array)
    {
        ...
        try {
            ...
            $maxPosition = ProductStatus::where('user_id', $array['user_id'])
                                    ->max('position') ?? 0;

            $data = new ProductStatus();
    ...
    public function import(UploadedFile $file)
    {
        try {
            ...
            // $processedCount = saveToDatabase($data, 'ProductStatus');
    ...
    public function export(?String $keyword = '', array $filters = [], String $perPage, String $sortBy = 'id', String $sortOrder = 'asc', String $type)
    {
        try {
            ...
            if (count($data['data']) > 0) {
                ...
                if ($type == 'excel') {
                    $fileExtension = ".xlsx";
                    return Excel::download(new ProductStatusesExport($data['data']), $fileName.$fileExtension);
                }
                elseif ($type == 'csv') {
                    $fileExtension = ".csv";
                    return Excel::download(new ProductStatusesExport($data['data']), $fileName.$fileExtension, \Maatwebsite\Excel\Excel::CSV);
                }
                elseif ($type == 'html') {
                    $fileExtension = ".html";
                    return Excel::download(new ProductStatusesExport($data['data']), $fileName.$fileExtension, \Maatwebsite\Excel\Excel::HTML);
                }
                elseif ($type == 'pdf') {
                    $fileExtension = ".pdf";
                    return Excel::download(new ProductStatusesExport($data['data']), $fileName.$fileExtension, \Maatwebsite\Excel\Excel::TCPDF);
                }
                else {
                    return [
                        'code' => 400,
