...
class InputSchoolSearch extends Component
{
    use WithPagination;
    ...
    {
        $badge = School::select('id','name','logo_path')->find($id);
        if (!$badge) return;
    ...
    {
        $data = School::when($this->search, function ($query) {
        ...
        if (count($this->selected) > 0) {
            $selectedBadges = School::whereIn('id', $this->selected)
                ->select(['id','name','logo_path'])
                ->get()
                ->keyBy('id');
        }

        return view('livewire.input-school-search', [
            'data' => $data,
