...
class InputSchoolClassSearch extends Component
{
    use WithPagination;
    ...
    {
        $badge = SchoolClass::select('id','name','thumbnail_icon')->find($id);
        if (!$badge) return;
    ...
    {
        $data = SchoolClass::when($this->search, function ($query) {
        ...
        if (count($this->selected) > 0) {
            $selectedBadges = SchoolClass::whereIn('id', $this->selected)
                ->select(['id','name','thumbnail_icon'])
                ->get()
                ->keyBy('id');
        }

        return view('livewire.input-school-class-search', [
            'data' => $data,
