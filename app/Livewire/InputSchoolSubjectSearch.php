...
class InputSchoolSubjectSearch extends Component
{
    use WithPagination;
    ...
    {
        $badge = SchoolSubject::select('id','name','thumbnail_icon')->find($id);
        if (!$badge) return;
    ...
    {
        $data = SchoolSubject::when($this->search, function ($query) {
        ...
        if (count($this->selected) > 0) {
            $selectedBadges = SchoolSubject::whereIn('id', $this->selected)
                ->select(['id','name','thumbnail_icon'])
                ->get()
                ->keyBy('id');
        }

        return view('livewire.input-school-subject-search', [
            'data' => $data,
