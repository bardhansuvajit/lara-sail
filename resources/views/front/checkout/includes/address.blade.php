    ...
    @if ( count($shippingAddresses) > 0 )
        <div class="w-full mt-4">
            @foreach ($shippingAddresses as $address)
                <x-front.radio-input-button id="addressId{{$address->id}}" name="shipping_address_id" value="{{$address->id}}" :checked="$address->is_default == 1" class="shipping-address" labelClass="mb-2">
            ...
            @if ( count($billingAddresses) > 0 )
                ...
                @foreach ($billingAddresses as $billing_address)
                    <x-front.radio-input-button id="addressId{{$billing_address->id}}" name="billing_address_id" value="{{$billing_address->id}}" :checked="$billing_address->is_default == 1" class="billing-address" labelClass="mb-2">
                        <div class="{{FD['rounded']}} shadow-sm dark:border-gray-700">
                            <div class="flex justify-between">
                                <div>
