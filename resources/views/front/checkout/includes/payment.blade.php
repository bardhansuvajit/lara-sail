            ...
            <div class="flex flex-col space-y-4">
                <div class="flex space-x-4">
                    @if ($paymentData['codEnable'] == 1)
                        <div>
                            <label class="flex items-start gap-3 p-3 border-2 border-gray-300 dark:border-gray-700/50 {{FD['rounded']}} cursor-pointer has-[:checked]:bg-gray-100 has-[:checked]:dark:bg-gray-600 has-[:checked]:border-gray-900 has-[:checked]:dark:border-gray-100">
                                <input type="radio" id="online_payment" name="payment_method" value="online" class="hidden md:inline-block mt-1 accent-primary-600 dark:accent-red-500" checked />
                    ...
                    @if ($paymentData['codEnable'] == 1)
                        <div>
                            <label class="flex items-start gap-3 p-3 border-2 border-gray-300 dark:border-gray-700/50 {{FD['rounded']}} cursor-pointer has-[:checked]:bg-gray-100 has-[:checked]:dark:bg-gray-600 has-[:checked]:border-gray-900 has-[:checked]:dark:border-gray-100">
                                <input type="radio" id="pay_on_delivery" name="payment_method" value="cod" class="hidden md:inline-block mt-1 accent-primary-600 dark:accent-red-500" />
                                <div class="flex flex-col {{FD['text-1']}} text-gray-700">
                ...
                <div>
                    <button type="submit" class="flex w-40 items-center justify-center {{FD['rounded']}} bg-primary-700 px-5 py-2.5 {{FD['text']}} font-medium text-white hover:bg-primary-800 focus:outline-none focus:ring-4 focus:ring-primary-300 dark:bg-primary-600 dark:hover:bg-primary-700 dark:focus:ring-primary-800">
                        Place Order
                    </button>
                </div>
            </div>
