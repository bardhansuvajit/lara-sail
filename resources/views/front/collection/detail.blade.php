        ...
        <header class="bg-gray-100 dark:bg-gray-900 {{ FD['rounded'] }}">
            ...
            <nav class="{{ FD['text-0'] }} text-gray-500 mt-2 mb-1" aria-label="breadcrumb">
                <ol class="flex items-center gap-2">
                    ...
                    <li>/</li>
                    <li><a href="{{ route('front.collection.index') }}" class="hover:underline text-gray-500 dark:text-gray-500">Collection</a></li>
                    <li>/</li>
                    <li><span class="text-gray-800 font-medium dark:text-gray-300">{{ $collection->title }}</span></li>
                </ol>
            </nav>
            ...
            <div class="grid grid-cols-1 items-center mt-2">
                <div class="lg:col-span-3">
                    <h1 class="text-sm md:text-lg font-extrabold leading-tight">{{ $collection->title }}</h1>

                    @if($collection->short_description)
                        <p class="{{ FD['text-0'] }} sm:text-xs text-gray-600 dark:text-gray-400 line-clamp-2">{{ $collection->short_description }}</p>
                    @endif
                </div>
            </div>
        </header>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-4">
            ...
            <div id="category-filter-template" class="hidden" aria-hidden="true">
                <div id="category-filter-root">
                    @include('layouts.front.includes.collection-detail-filter')
                </div>
            </div>
        ...
        @push('head')
            <script type="application/ld+json">
                {!! json_encode([
                    ...
                    "description" => $collection->short_description ?? Str::limit(strip_tags($collection->description ?? ''), 160),
                ]) !!}
            </script>
        @endpush
    </div>
