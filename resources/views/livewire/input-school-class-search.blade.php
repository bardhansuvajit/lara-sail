<div 
    ...
    wire:ignore.self
>
    <x-admin.input-label for="product_id" :value="__('Class')" />
    ...
    @if($mode === 'single')
        <input type="hidden" name="class_id" value="{{ $product_id ?? ($selected[0] ?? '') }}">
        <input type="hidden" name="class_title" value="{{ $product_title ?? '' }}">
    @else
        @foreach($selected as $id)
            @php $class = $selectedBadges[$id] ?? null; @endphp
            <input type="hidden" name="class_ids[]" value="{{ $id }}">
            <input type="hidden" name="class_titles[]" value="{{ $class?->name ?? '' }}">
            <input type="hidden" name="class_icons[]" value="{{ $class?->thumbnail_icon ?? '' }}">
        @endforeach
    @endif

    <x-admin.input-error :messages="$errors->get('class_ids')" class="mt-2" />
</div>