<div 
    ...
    wire:ignore.self
>
    <x-admin.input-label for="product_id" :value="__('Board')" />
    ...
    <div class="mt-2 flex flex-wrap gap-2" wire:key="selected-chips-{{ md5(json_encode($selected)) }}">
        @foreach($selected as $id)
            ...
            <div class="p-1 inline-flex items-center gap-2 rounded shadow-sm bg-gray-100 dark:bg-gray-700 text-gray-800 dark:text-gray-200" wire:key="selected-{{ $id }}">
                <div class="w-24">
                    {!! $class?->thumbnail_icon ?? 'here' !!}
                </div>

                <span class="px-1 py-0.5 h-5 text-xs line-clamp-1">{{ $class?->name ?? 'Board #' . $id }}</span>
    ...
    @if($mode === 'single')
        <input type="hidden" name="board_id" value="{{ $product_id ?? ($selected[0] ?? '') }}">
        <input type="hidden" name="board_title" value="{{ $product_title ?? '' }}">
    @else
        @foreach($selected as $id)
            @php $class = $selectedBadges[$id] ?? null; @endphp
            <input type="hidden" name="board_ids[]" value="{{ $id }}">
            <input type="hidden" name="board_titles[]" value="{{ $class?->name ?? '' }}">
            <input type="hidden" name="board_icons[]" value="{{ $class?->thumbnail_icon ?? '' }}">
        @endforeach
    @endif

    <x-admin.input-error :messages="$errors->get('board_ids')" class="mt-2" />
</div>