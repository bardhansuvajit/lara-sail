<div 
    ...
    wire:ignore.self
>
    <x-admin.input-label for="product_id" :value="__('Subject')" />
    ...
    @if($mode === 'single')
        <input type="hidden" name="subject_id" value="{{ $product_id ?? ($selected[0] ?? '') }}">
        <input type="hidden" name="subject_title" value="{{ $product_title ?? '' }}">
    @else
        @foreach($selected as $id)
            @php $class = $selectedBadges[$id] ?? null; @endphp
            <input type="hidden" name="subject_ids[]" value="{{ $id }}">
            <input type="hidden" name="subject_titles[]" value="{{ $class?->name ?? '' }}">
            <input type="hidden" name="subject_icons[]" value="{{ $class?->thumbnail_icon ?? '' }}">
        @endforeach
    @endif

    <x-admin.input-error :messages="$errors->get('subject_ids')" class="mt-2" />
</div>