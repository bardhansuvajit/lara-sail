        ...
        <div class="flex space-x-2 justify-end">
            ...
            <x-admin.button 
                ...
                x-on:click.prevent="
                    ...
                    $dispatch('set-model', 'ProductVariationAttributeValue');
                    $dispatch('set-route', '{{ route('admin.product.variation.attribute.value.import') }}');
                ">
                @slot('icon')
            ...
            <x-admin.button 
                ...
                x-on:click.prevent="
                    ...
                    $dispatch('set-route', '{{ route('admin.product.variation.attribute.value.export', 'csv') }}');
                ">
                @slot('icon')
        ...
        <form action="" method="get">
            <div class="grid grid-cols-10 gap-4 py-4">
                            ...
                            <div class="flex space-x-1">
                                ...
                                <x-admin.button-icon
                                    ...
                                    x-on:click.prevent="
                                        ...
                                        $dispatch('set-route', '{{ route('admin.product.variation.attribute.value.bulk') }}');
                ...
                <div class="col-span-8 flex flex-row space-x-1 justify-end items-end">
                    ...
                    <input type="hidden" name="attributeId" value="{{request()->input('attributeId', '')}}">
                ...
                <tbody id="sortable-container" data-route="{{ route('admin.product.variation.attribute.value.position') }}">
                    @forelse ($data as $item)
                        <tr class="border-b border-gray-100 dark:border-gray-600 hover:bg-gray-100 dark:hover:bg-gray-700" data-id="{{$item->id}}">
                            ...
                            <td scope="row" class="px-2 py-1 text-gray-900 dark:text-white">
                                @if ($item->attribute)
                                    <a href="{{route('admin.product.variation.attribute.edit', $item->attribute_id)}}" class="text-xs underline hover:no-underline">{{ $item->attribute->title }}</a>
                                @else
