                        ...
                        <tr class="border-b border-gray-100 dark:border-gray-600 hover:bg-gray-100 dark:hover:bg-gray-700" data-id="{{$item->id}}">
                            ...
                            <td scope="row" class="px-2 py-1 text-gray-900 dark:text-white">
                                <div class="flex space-x-2 items-center">
                                    @if($item->state->flag) <div class="w-8 h-8 overflow-hidden flex">{!! $item->state->flag !!}</div> @endif
                                    <div>
                                        <p class="text-xs font-bold">{{ $item->state->name }}</p>
                            ...
                            <td scope="row" class="px-2 py-1 text-gray-700 dark:text-gray-300 font-medium">
                                @if ($item->discount_type == "percentage")
                                    <p class="text-xs">{{ formatIndianMoney($item->value) }}% Off upto {{ $item->state->currency_symbol }}{{ formatIndianMoney($item->max_discount_amount) }}</p>
                                    <p class="text-xs">Minimum Cart value {{ $item->state->currency_symbol }}{{ formatIndianMoney($item->min_cart_value) }}</p>
                                @elseif ($item->discount_type == "fixed")
                                    <p class="text-xs">Fixed {{ $item->state->currency_symbol }}{{ formatIndianMoney($item->value) }} Off</p>
                                    <p class="text-xs">Minimum Cart value {{ $item->state->currency_symbol }}{{ formatIndianMoney($item->min_cart_value) }}</p>
                                @else
                                    <p class="text-xs">Free Shipping on Minimum Cart value {{ $item->state->currency_symbol }}{{ formatIndianMoney($item->min_cart_value) }}</p>
                                @endif
                            </td>
                            <td scope="row" class="px-2 py-1 text-gray-700 dark:text-gray-300 font-medium">
                                <p class="text-xs">
