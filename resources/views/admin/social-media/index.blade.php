...
>
    <section class="sm:rounded-lg overflow-hidden px-1 py-2">
        ...
        <div class="flex space-x-2 justify-end">
            <x-admin.button
                element="a"
                :href="route('admin.website.social.media.create')">
                @slot('icon')
            ...
            <x-admin.button 
                ...
                x-on:click.prevent="
                    ...
                    $dispatch('set-model', 'SocialMedia');
                    $dispatch('set-route', '{{ route('admin.website.social.media.import') }}');
                ">
                @slot('icon')
            ...
            <x-admin.button 
                ...
                x-on:click.prevent="
                    ...
                    $dispatch('set-route', '{{ route('admin.website.social.media.export', 'csv') }}');
                ">
                @slot('icon')
                            ...
                            <div class="flex space-x-1">
                                ...
                                <x-admin.button-icon
                                    ...
                                    x-on:click.prevent="
                                        ...
                                        $dispatch('set-route', '{{ route('admin.website.social.media.bulk') }}');
            ...
            <table class="w-full text-xs text-left text-gray-500 dark:text-gray-400">
                <thead class="text-xs text-gray-700 uppercase bg-gray-100 dark:bg-gray-700 dark:text-gray-400">
                    <tr class="h-8">
                        ...
                        <th scope="col" class="px-2 py-1">Colored Icon</th>
                        <th scope="col" class="px-2 py-1">Base Icon</th>
                        <th scope="col" class="px-2 py-1">URL</th>
                        <th scope="col" class="px-2 py-1 text-end">Action</th>
                    </tr>
                </thead>
                <tbody id="sortable-container" data-route="{{ route('admin.website.social.media.position') }}">
                    @forelse ($data as $item)
                        <tr class="border-b border-gray-100 dark:border-gray-700 hover:bg-gray-100 dark:hover:bg-gray-700" data-id="{{$item->id}}">
                            ...
                            <td scope="row" class="px-2 py-1">
                                @if ($item->url)
                                    <a href="{{ $item->url }}" class="text-xs font-bold underline text-primary-400 dark:text-primary-500" target="_blank">{{ $item->url }}</a>
                                @else
                                    <P class="text-gray-400 dark:text-gray-500">NA</P>
                                @endif
                            </td>
                                ...
                                <div class="flex space-x-2 items-center justify-end">
                                    ...
                                    <x-admin.button-icon
                                        ...
                                        x-on:click.prevent="
                                            ...
                                            $dispatch('data-icon_colored', '{{ $item->icon_colored }}');
                                            $dispatch('data-icon_base', '{{ $item->icon_base }}');
                                            $dispatch('data-url', '{{ $item->url }}');
                                        " >
                                        @slot('icon')
                                    ...
                                    <x-admin.button-icon
                                        element="a"
                                        tag="secondary"
                                        :href="route('admin.website.social.media.edit', $item->id)"
                                        title="Edit"
                                        class="border" >
                                        @slot('icon')
                                    ...
                                    <x-admin.button-icon
                                        ...
                                        x-on:click.prevent="
                                            ...
                                            $dispatch('set-delete-route', '{{ route('admin.website.social.media.delete', $item->id) }}')
                                        " >
                                        @slot('icon')
                                            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 -960 960 960" fill="currentColor"><path d="M280-120q-33 0-56.5-23.5T200-200v-520h-40v-80h200v-40h240v40h200v80h-40v520q0 33-23.5 56.5T680-120H280Zm400-600H280v520h400v-520ZM360-280h80v-360h-80v360Zm160 0h80v-360h-80v360ZM280-720v520-520Z"/></svg>
                                        @endslot
    ...
    <x-admin.sidebar name="quick-data-view" maxWidth="sm" direction="right" header="Quick View" focusable>
        <div 
            class="p-4"
            x-data="{image: '', title: '', icon_colored: '', icon_base: '', url: ''}"
            x-on:data-image.window="image = $event.detail"
            x-on:data-title.window="title = $event.detail"
            x-on:data-icon_colored.window="icon_colored = $event.detail"
            x-on:data-icon_base.window="icon_base = $event.detail"
        ...
        >
            ...
            <h5 class="text-xs font-bold mb-1">Colored Icon</h5>
            <div class="h-20 w-20 mb-3" x-html="icon_colored"></div>

            <h5 class="text-xs font-bold mb-1">Base Icon</h5>
            <div class="h-20 w-20 mb-3" x-html="icon_base"></div>

            <h5 class="text-xs font-bold mb-1">URL</h5>
            <p class="text-sm mb-3" x-text="url"></p>
