...
>
    ...
    <section class="sm:rounded-lg overflow-hidden px-1 py-2">
        ...
        <div class="flex space-x-2 justify-end">
            <x-admin.button
                element="a"
                :href="route('admin.master.state.create')">
                @slot('icon')
            ...
            <x-admin.button 
                ...
                x-on:click.prevent="
                    ...
                    $dispatch('set-model', 'State');
                    $dispatch('set-route', '{{ route('admin.master.state.import') }}');
                ">
                @slot('icon')
            ...
            <x-admin.button 
                ...
                x-on:click.prevent="
                    ...
                    $dispatch('set-route', '{{ route('admin.master.state.export', 'csv') }}');
                ">
                @slot('icon')
                            ...
                            <div class="flex space-x-1">
                                ...
                                <x-admin.button-icon
                                    ...
                                    x-on:click.prevent="
                                        ...
                                        $dispatch('set-route', '{{ route('admin.master.state.bulk') }}');
                ...
                <tbody>
                    @forelse ($data as $item)
                        <tr class="border-b border-gray-100 dark:border-gray-600 hover:bg-gray-100 dark:hover:bg-gray-700">
                            ...
                            <td scope="row" class="px-2 py-1 text-gray-500">
                                <p class="text-xs">
                                    Shipping: <span class="text-gray-900 dark:text-white">{{ ($item->shipping_availability == 1) ? 'YES' : 'NA' }}</span>
                                    <span class="px-2">|</span>
                                    COD: <span class="text-gray-900 dark:text-white">{{ ($item->cash_on_delivery_availability == 1) ? 'YES' : 'NA' }}</span>
                                </p>
                            </td>
                            <td scope="row" class="px-2 py-1 text-gray-500">
                                <div class="flex space-x-2 items-center justify-end">
                                    ...
                                    <x-admin.button-icon
                                        element="a"
                                        tag="secondary"
                                        :href="route('admin.master.state.edit', $item->id)"
                                        title="Edit"
                                        class="border" >
                                        @slot('icon')
                                    ...
                                    <x-admin.button-icon
                                        ...
                                        x-on:click.prevent="
                                            ...
                                            $dispatch('set-delete-route', '{{ route('admin.master.state.delete', $item->id) }}')" >
                                        @slot('icon')
                                            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 -960 960 960" fill="currentColor"><path d="M280-120q-33 0-56.5-23.5T200-200v-520h-40v-80h200v-40h240v40h200v80h-40v520q0 33-23.5 56.5T680-120H280Zm400-600H280v520h400v-520ZM360-280h80v-360h-80v360Zm160 0h80v-360h-80v360ZM280-720v520-520Z"/></svg>
                                        @endslot
                                    </x-admin.button-icon>
