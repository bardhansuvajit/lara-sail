...
>
    ...
    <section class="sm:rounded-lg overflow-hidden px-1 py-2">
        ...
        <div class="flex space-x-2 justify-end">
            <x-admin.button
                element="a"
                :href="route('admin.website.ad.section.create')">
                @slot('icon')
            ...
            <x-admin.button 
                ...
                x-on:click.prevent="
                    ...
                    $dispatch('set-route', '{{ route('admin.website.ad.section.import') }}');
                ">
                @slot('icon')
            ...
            <x-admin.button 
                ...
                x-on:click.prevent="
                    ...
                    $dispatch('set-route', '{{ route('admin.website.ad.section.export', 'csv') }}');
                ">
                @slot('icon')
                            ...
                            <div class="flex space-x-1">
                                ...
                                <x-admin.button-icon
                                    ...
                                    x-on:click.prevent="
                                        ...
                                        $dispatch('set-route', '{{ route('admin.website.ad.section.bulk') }}');
            ...
            <table class="w-full text-xs text-left text-gray-500 dark:text-gray-400">
                <thead class="text-xs text-gray-700 uppercase bg-gray-100 dark:bg-gray-700 dark:text-gray-400">
                    <tr class="h-8">
                        ...
                        <th scope="col" class="px-2 py-1">Pages</th>
                        <th scope="col" class="px-2 py-1">Item</th>
                    ...
                    @forelse ($data as $item)
                        <tr class="border-b border-gray-100 dark:border-gray-600 hover:bg-gray-100 dark:hover:bg-gray-700" data-id="{{$item->id}}">
                            ...
                            <td scope="row" class="px-2 py-1 text-gray-900 dark:text-white">
                                <p class="text-xs font-bold italic capitalize">{{ $item->pages }}</p>
                            </td>
                            <td scope="row" class="px-2 py-1 text-gray-900 dark:text-white">
                                <a href="{{ route('admin.website.ad.item.index', ['ad_section_id' => $item->id]) }}" class="text-xs font-bold underline hover:no-underline">
                            ...
                            <td scope="row" class="px-2 py-1 text-gray-900 dark:text-white">
                                <p class="text-xs font-bold">{{ $item->type }}</p>
                            </td>
                                ...
                                <div class="flex space-x-2 items-center justify-end">
                                    ...
                                    <x-admin.button-icon
                                        ...
                                        x-on:click.prevent="
                                            ...
                                            $dispatch('data-pages', '{{ $item->pages }}');
                                            $dispatch('data-name', '{{ $item->name }}');
                                            $dispatch('data-type', '{{ $item->type }}');
                                        " >
                                        @slot('icon')
                                    ...
                                    <x-admin.button-icon
                                        element="a"
                                        tag="secondary"
                                        :href="route('admin.website.ad.section.edit', $item->id)"
                                        title="Edit"
                                        class="border" >
                                        @slot('icon')
                                    ...
                                    <x-admin.button-icon
                                        ...
                                        x-on:click.prevent="
                                            ...
                                            $dispatch('set-delete-route', '{{ route('admin.website.ad.section.delete', $item->id) }}')
                                        " >
                                        @slot('icon')
                                            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 -960 960 960" fill="currentColor"><path d="M280-120q-33 0-56.5-23.5T200-200v-520h-40v-80h200v-40h240v40h200v80h-40v520q0 33-23.5 56.5T680-120H280Zm400-600H280v520h400v-520ZM360-280h80v-360h-80v360Zm160 0h80v-360h-80v360ZM280-720v520-520Z"/></svg>
                                        @endslot
    ...
    <x-admin.sidebar name="quick-data-view" maxWidth="sm" direction="right" header="Quick View" focusable>
        <div 
            class="p-4"
            x-data="{pages: '', name: '', type: ''}"
            x-on:data-pages.window="pages = $event.detail"
            x-on:data-name.window="name = $event.detail"
            x-on:data-type.window="type = $event.detail"
        >
            <h5 class="text-xs font-bold mb-1">Pages</h5>
            <p class="text-sm mb-3" x-text="pages"></p>

            <h5 class="text-xs font-bold mb-1">Name</h5>
            <p class="text-sm mb-3" x-text="name"></p>

            <h5 class="text-xs font-bold mb-1">Type</h5>
            <p class="text-sm mb-3" x-text="type"></p>
