...
>
    ...
    <section class="sm:rounded-lg overflow-hidden px-1 py-2">
        ...
        <div class="flex space-x-2 justify-end">
            <x-admin.button
                element="a"
                :href="route('admin.website.content.page.create')">
                @slot('icon')
            ...
            <x-admin.button 
                ...
                x-on:click.prevent="
                    ...
                    $dispatch('set-model', 'ContentPage');
                    $dispatch('set-route', '{{ route('admin.website.content.page.import') }}');
                ">
                @slot('icon')
            ...
            <x-admin.button 
                ...
                x-on:click.prevent="
                    ...
                    $dispatch('set-route', '{{ route('admin.website.content.page.export', 'csv') }}');
                ">
                @slot('icon')
                    ...
                    <div class="flex space-x-1 items-end">
                        ...
                        <div class="w-max">
                            ...
                            <x-admin.input-select 
                                id="sortBy" 
                                name="sortBy"
                                :title="request()->input('sortBy', 'Title')"
                            ...
                            <div class="flex space-x-1">
                                ...
                                <x-admin.button-icon
                                    ...
                                    x-on:click.prevent="
                                        ...
                                        $dispatch('set-route', '{{ route('admin.website.content.page.bulk') }}');
                ...
                <tbody>
                    @forelse ($data as $item)
                        <tr class="border-b border-gray-100 dark:border-gray-700 hover:bg-gray-100 dark:hover:bg-gray-700">
                            ...
                            <td scope="row" class="px-2 py-1 text-gray-500 w-96 line-clamp-3">
                                ...
                                <div class="flex space-x-2 items-center justify-end">
                                    ...
                                    <x-admin.button-icon
                                        ...
                                        x-on:click.prevent="
                                            ...
                                            $dispatch('data-slug', '{{ htmlspecialchars($item->slug, ENT_QUOTES) }}');
                                            {{-- $dispatch('data-content', '{{ json_encode(htmlspecialchars($item->content, ENT_QUOTES)) }}'); --}}
                                            $dispatch('data-content', {{ json_encode(nl2br(htmlspecialchars($item->content, ENT_QUOTES))) }});
                                            $dispatch('data-meta_title', '{{ htmlspecialchars($item->meta_title, ENT_QUOTES) }}');
                                            $dispatch('data-meta_description', '{{ htmlspecialchars($item->meta_description, ENT_QUOTES) }}');
                                    ...
                                    <x-admin.button-icon
                                        element="a"
                                        tag="secondary"
                                        :href="route('admin.website.content.page.edit', $item->id)"
                                        title="Edit"
                                        class="border" >
                                        @slot('icon')
                                    ...
                                    <x-admin.button-icon
                                        ...
                                        x-on:click.prevent="
                                            ...
                                            $dispatch('data-title', '{{ htmlspecialchars($item->title, ENT_QUOTES) }}');
                                            $dispatch('set-delete-route', '{{ route('admin.website.content.page.delete', $item->id) }}')" >
                                        @slot('icon')
                                            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 -960 960 960" fill="currentColor"><path d="M280-120q-33 0-56.5-23.5T200-200v-520h-40v-80h200v-40h240v40h200v80h-40v520q0 33-23.5 56.5T680-120H280Zm400-600H280v520h400v-520ZM360-280h80v-360h-80v360Zm160 0h80v-360h-80v360ZM280-720v520-520Z"/></svg>
                                        @endslot
                                    </x-admin.button-icon>
    ...
    <x-admin.sidebar name="quick-data-view" maxWidth="sm" direction="right" header="Quick View" focusable>
        <div 
            class="p-4"
            x-data="{title: '', slug: '', content: '', meta_title: '', meta_description: '', status: ''}"
            x-on:data-title.window="title = $event.detail"
            x-on:data-slug.window="slug = $event.detail"
            x-on:data-content.window="content = $event.detail"
            x-on:data-meta_title.window="meta_title = $event.detail"
            x-on:data-meta_description.window="meta_description = $event.detail"
            x-on:data-status.window="status = $event.detail"
        >
            <h5 class="text-xs font-bold mb-1 text-primary-500 dark:text-primary-300">Name</h5>
            <p class="text-sm mb-3" x-text="title"></p>

            <h5 class="text-xs font-bold mb-1 text-primary-500 dark:text-primary-300">Slug</h5>
            <p class="text-sm mb-3" x-text="slug"></p>

            <h5 class="text-xs font-bold mb-1 text-primary-500 dark:text-primary-300">Content</h5>
            <p class="text-sm mb-3" x-html="content"></p>

            <h5 class="text-xs font-bold mb-1 text-primary-500 dark:text-primary-300">Meta Title</h5>
            <p class="text-sm mb-3" x-text="meta_title"></p>

            <h5 class="text-xs font-bold mb-1 text-primary-500 dark:text-primary-300">Meta Description</h5>
            <p class="text-sm mb-3" x-text="meta_description"></p>

            <h5 class="text-xs font-bold mb-1 text-primary-500 dark:text-primary-300">Status</h5>
